...
from .offline.cache import transcription_cache_path, translation_cache_path, load_json
...
def offline(url: str = typer.Argument(..., help="URL YouTube de l'épisode"),
    ...
    logger.info('pipeline.complete', srt=str(srt), seconds=f"{time.time()-start:.2f}")
...
def inspect_cache(audio_file: Path):
...
@app.command('batch')
def batch(urls_file: Path = typer.Argument(..., help="Fichier texte: 1 URL par ligne"),
    ...
    summary={'success':[r for r in results if r[1].endswith('.srt')], 'failed':[r for r in results if not r[1].endswith('.srt')]}
    typer.echo(json.dumps(summary, indent=2, ensure_ascii=False))

@app.command('bench')
def bench(url: str = typer.Argument(...), loops: int = typer.Option(1, help="Nb répétitions")):
    times=[]
    ...
    typer.echo(f"Moyenne: {avg:.2f}s | Min: {min(times):.2f}s | Max: {max(times):.2f}s")
